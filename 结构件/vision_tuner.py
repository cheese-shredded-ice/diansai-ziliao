#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""RDK X5 v5 视觉与通信调试上位机。

界面沿用 v4/v6 调参器的工作方式：启动时不打开串口，只有点击
“串口：关”后才打开，并在打开期间发送 STM32 ASCII 视觉帧。
相机、识别、ROI、映射和速度参数均实时生效；左侧保留真实完整画面。
"""
...
MODULE_PATH = SCRIPT_DIR / "rdk_ball_stable_v5(2).py"
MODULE_SPEC = importlib.util.spec_from_file_location("rdk_ball_stable_v5_current", MODULE_PATH)
if MODULE_SPEC is None or MODULE_SPEC.loader is None:
    raise ImportError(f"无法加载视觉模块: {MODULE_PATH}")
V5 = importlib.util.module_from_spec(MODULE_SPEC)
MODULE_SPEC.loader.exec_module(V5)
CONFIG_PATH = SCRIPT_DIR / "ball_stable_config(2).json"
EXPORT_PATH = SCRIPT_DIR / "ball_stable_config(2)_export.json"
...
class App(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(left, text="v5 视觉调参  |  offset + velocity 联调", bg=BG, fg=ACCENT,
    ...
    def _toggle_serial(self) -> None:
        ...
        if ok and self.serial.ser is not None and self.serial.ser.is_open:
            ...
            self.serial_lbl.config(text=f"串口：已打开 {self.serial.port}（正在发送ASCII帧）", fg=ERR)
            self._set_status(f"串口已开 {self.serial.port}，将发送ASCII视觉帧", OK)
        else:
            self.serial.ser = None
            self._serial_on = False
            self._btn_serial.config(text="串口：关", bg=BTN_BG)
    ...
    def _tick(self) -> None:
        ...
        if roi is not None and seq != self._last_seq:
            ...
            frame = V5.build_frame(
                self._tx_frame_seq,
                signal["valid"],
                signal.get("offset_px", signal["offset"]),
                signal.get("speed_px_s", signal["velocity"]),
                int(self.config_data.get("signal", {}).get("offset_limit_px", 200)),
                int(self.config_data.get("signal", {}).get("speed_limit_px_s", 20000)),
            )
            self._tx_frame_seq = (self._tx_frame_seq + 1) & 0xFFFFFFFF
            if self._serial_on:
            ...
            if signal["valid"]:
                self.signal_lbl.config(text=(f"valid=1  x={signal.get('position_px', 0.0):.1f}  target={signal.get('target_x_px', 0.0):.1f}  offset_px={signal.get('offset_px', signal['offset']):+.1f}  speed_px_s={signal.get('speed_px_s', signal['velocity']):+.1f}\n"
                                             f"raw_speed={signal.get('raw_speed_px_s', signal['raw_velocity']):+.1f}  dt={signal['dt']:.3f}  TX={frame.decode('ascii').rstrip()}"))
            else:
                self.signal_lbl.config(text=f"valid=0  offset_px=0  speed_px_s=0  TX={frame.decode('ascii').rstrip()}（丢球/跳变）")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RDK X5 视觉调参与 STM32 ASCII 协议联调")
    parser.add_argument("--config", type=Path, default=CONFIG_PATH)
    args = parser.parse_args()
    CONFIG_PATH = args.config
    EXPORT_PATH = CONFIG_PATH.with_name(CONFIG_PATH.stem + "_export.json")
    App().mainloop()
